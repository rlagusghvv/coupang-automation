...
### 17:34 KST — data (재현 포인트 추가: 추천 다건 업로드)
- 대표 관측: 추천 리스트에서 **여러 상품을 한 번에 업로드**할 때 오류가 발생
- 시사점: 단건 URL 업로드는 OK, 다건 업로드(배치/병렬/연속 요청) 경로에서 레이트리밋/세션/DB 락/큐 처리 문제 가능
- 다음: 다건 업로드 UX를 "큐(순차)"로 고정 + 진행률/실패 아이템만 재시도 + 서버측 동시성 제한으로 안정화
